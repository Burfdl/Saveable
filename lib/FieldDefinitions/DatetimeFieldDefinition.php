<?php
	...
	class DatetimeFieldDefinition extends FieldDefinition
	{
		...
		public static function manufacture($name = null) {
			return new DatetimeFieldDefinition($name);
		}
		...
		public function matchesSqlType($fieldDetails) {
			return preg_match("/COMMENT '([^\\]\\'|[^'])*~~".__CLASS__."~~([^\\]\\'|[^'])*'/", $fieldDetails) ||
				preg_match("/^datetime/i", $fieldDetails);
		}
		
		public function fromSQLCreateFieldLine($name, $line) {
			$field = new DatetimeFieldDefinition($name);
			
			$matches = array();
			if (preg_match("/^datetime/i", $line, $matches))
			{
