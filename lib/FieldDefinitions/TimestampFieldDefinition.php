<?php
	...
	class TimestampFieldDefinition extends FieldDefinition
	{
		...
		public static function manufacture($name = null) {
			return new TimestampFieldDefinition($name);
		}
		...
		public function matchesSqlType($fieldDetails) {
			return preg_match("/COMMENT '([^\\]\\'|[^'])*~~".__CLASS__."~~([^\\]\\'|[^'])*'/", $fieldDetails) ||
				preg_match("/^timestamp/i", $fieldDetails);
		}
		
		public function fromSQLCreateFieldLine($name, $line) {
			$field = new TimestampFieldDefinition($name);
			
			$matches = array();
			if (preg_match("/^timestamp/i", $line, $matches))
			{
