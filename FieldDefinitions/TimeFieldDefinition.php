<?php
	...
	class TimeFieldDefinition extends FieldDefinition
	{
		...
		public static function manufacture($name = null) {
			return new TimeFieldDefinition($name);
		}
		...
		public function matchesSqlType($fieldDetails) {
			return preg_match("/COMMENT '([^\\]\\'|[^'])*~~".__CLASS__."~~([^\\]\\'|[^'])*'/", $fieldDetails) ||
				preg_match("/^time( |,|$)/i", $fieldDetails);
		}
		
		public function fromSQLCreateFieldLine($name, $line) {
			$field = new TimeFieldDefinition($name);
			
			$matches = array();
			if (preg_match("/^time/i", $line, $matches))
			{
		...
		public function isValidValue($value) {
			return preg_match("/^([0-9]? ?[0-9]{2}:([0-9]{2}(:[0-9]{2}(.[0-9]+)?)?)?)|(NOW\(\)|CURRENT_DATE)$/i", $value);
		}
